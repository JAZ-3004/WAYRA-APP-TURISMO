...
function AppContent() {
  ...
  // PWA Install states
  // deferredPrompt: solo existe cuando el navegador detecta que la app NO está instalada
  const [deferredPrompt, setDeferredPrompt] = useState<any>(null);
  // isStandalone: true cuando la app ya está instalada y corriendo como PWA
  ...
  useEffect(() => {
    ...
    const checkStandalone = () => {
      try {
        ...
        console.warn('[PWA] Error detectando modo standalone:', err);
      }
    };

    checkStandalone();

    // Escuchar cambios (ej. usuario la instala mientras navega)
    ...
    try {
      if (typeof window !== 'undefined' && window.matchMedia) {
        ...
        handleModeChange = (e: MediaQueryListEvent) => {
          ...
          if (e.matches) setDeferredPrompt(null); // ya instalada, limpiar prompt
        };
        mql.addEventListener('change', handleModeChange);
      }
    } catch (err) {}

    // El navegador solo dispara este evento si la app NO está instalada y es instalable
    ...
    // Cuando el usuario completa la instalación desde el banner del navegador
    ...
    return () => {
      try { if (mql && handleModeChange) mql.removeEventListener('change', handleModeChange); } catch (e) {}
      window.removeEventListener('beforeinstallprompt', handleBeforeInstallPrompt);
      window.removeEventListener('appinstalled', handleAppInstalled);
    };
  }, []);

  // Lanza el diálogo nativo de instalación del navegador — sin guías, sin modales
        ...
        <AnimatePresence>
          {deferredPrompt && !isStandalone && (
            <motion.div
              ...
              className="mb-8"
            >
              <div className="bg-gradient-to-r from-amber-900 to-amber-950 text-white p-4 rounded-[2rem] shadow-xl border border-amber-700/20 relative overflow-hidden">
                ...
                <div className="relative z-10 flex items-center gap-3">
                  ...
                  <img
                    src="/icon-192.png"
                    alt="Wayra"
                    className="w-12 h-12 rounded-2xl shadow-lg shrink-0 border-2 border-amber-600/30"
                  />
                  <div className="flex-1 min-w-0">
                    <p className="font-logo font-black uppercase text-[10px] tracking-widest text-amber-400 leading-none">Instala Wayra</p>
                    <p className="text-[10px] text-amber-100/60 font-sans mt-1 leading-snug">
                      Agrégala a tu pantalla de inicio y úsala como app nativa.
                    </p>
                  </div>
                  <div className="flex flex-col gap-1.5 shrink-0 items-end">
                    <button
                      onClick={handleInstallApp}
                      className="px-4 py-2 bg-amber-500 text-amber-950 font-sans font-black uppercase tracking-wider text-[9px] rounded-xl hover:bg-amber-400 active:scale-95 transition-all shadow-lg cursor-pointer"
                    >
                      Instalar
                    </button>
                    <button
                      onClick={handleCloseBanner}
                      className="text-[9px] font-sans font-semibold text-amber-200/40 hover:text-amber-200/70 cursor-pointer px-1 py-0.5 transition-colors"
                    >
                      Ahora no
                    </button>
                  </div>
